...
const generateAnalyticsJSON = docs => {
    ...
    let batches = docs.map( doc => doc.data().batch )

    let sortedBatches = [ ...new Set(batches)].sort((a,b) => parseInt(a.replace(/[ws]/i,'')) - parseInt(b.replace(/[ws]/i,'')))

    const totalFunding = (d, b) => d.filter( doc => doc.data().batch == b).reduce( (funding,doc) => funding + doc.data().funding, 0)

    var sortedFunding = sortedBatches.map( batch => totalFunding(docs, batch) )

    const totalCompanies = (d, b) => d.filter( doc => doc.data().batch == b).length

    var sortedCount = sortedBatches.map( batch => totalCompanies(docs, batch) )

    const totalStatus = (d,b,s) => d.filter( doc => doc.data().batch == b && doc.data().status == s).length

    var sortedLive = sortedBatches.map( batch => totalStatus(docs, batch, 'Live') )
    var sortedExited = sortedBatches.map( batch => totalStatus(docs, batch, 'Exited') )
    var sortedDead = sortedBatches.map( batch => totalStatus(docs, batch, 'Dead') )

    const totalLevel = (d,b,min,max) => d.filter( doc => doc.data().batch == b && doc.data().funding > min && doc.data().funding <= max).length

    var sortedMega = sortedBatches.map( batch => totalLevel(docs, batch, 10.0, 99999999.0) )
    var sortedMini = sortedBatches.map( batch => totalLevel(docs, batch, 5.0, 10.0) )
    var sortedSeed = sortedBatches.map( batch => totalLevel(docs, batch, 0.0, 5.0) )
    var sortedNone = sortedBatches.map( batch => totalLevel(docs, batch, -1.0, 0.0) )
